...
(() => {
  ...
  s.addText("Основной офис — в Москве. В 2026 году вошла в состав Горно-строительной компании. Инженеры и технологи работают в отрасли по 10–25 лет, с опытом проектов в России, странах СНГ и за рубежом.", {
    x: ML, y: 4.4, w: 6.4, h: 1.6, fontFace: BF, fontSize: 14, italic: true, color: STEEL, valign: "top", margin: 0, lineSpacingMultiple: 1.25 });
...
(() => {
  ...
  const tx = 6.3;
  s.addText("Разработка технологий и производство оборудования для получения обессоленной воды для технологических процессов. Отдельное направление — опреснение морской воды.", {
...
(() => {
  ...
  title(s, "Технологии");
  const methods = [
    ["Баромембранные", "обратный осмос, нанофильтрация, ультра- и микрофильтрация; опреснение морской воды"],
...
(() => {
  ...
  s.addText("Реализованные проекты в России, странах СНГ и за рубежом.", {
    x: ML, y: 1.35, w: 7.6, h: 0.5, fontFace: BF, fontSize: 15, color: DARK, valign: "top", margin: 0 });
  photo(s, ML, 1.95, 7.6, 4.65, "Города поставок —\nМосква · Норильск · Байкальск · Североуральск ·\nИркутск · Тула · Волгоградская обл. · Казахстан");
...
(() => {
  ...
  const rows = [
    ...
    ["Основной офис", "Москва, ул. Большая Академическая, 5с3"],
    ["Сайт", "gsk-tech.ru"],
...
pres.writeFile({ fileName: "/home/user/123/Презентация.pptx" }).then((f) => console.log("Saved:", f));
